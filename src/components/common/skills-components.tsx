...
const Modal = ({ skill, isOpen, onClose }: ModalProps) => (
    <JoyModal open={isOpen} onClose={onClose}>
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 400, boxShadow: 24, p: 4, borderRadius: 2, backgroundColor: '#212029', color: '#fff' }}>
            ...
            <Typography sx={{ color: '#fff' }}>{skillDescriptions[skill.name]}</Typography>
        </Box>
    </JoyModal>
);
    ...
    return (
        <>
            ...
            >
                ...
                <p className="text-neutral-300 text-sm font-medium text-center">{name}</p>
            </motion.div>
            <Modal
                skill={{ name, icon }}
                isOpen={isModalOpen}
